    ...
    <main class="main">
      <section class="flags">
        ...
        <article class="fact">
          ...
          <span class="fact__total"></span>
        </article>
      </section>
    </main>
    <footer class="fact__report">
      <?php include_once('checkLastQuery.php'); ?>
    </footer>
  </div>
  <script>
    function totalCasesByCountry(covidDataByCountry) {
      let totalConfirmed = 0
      let totalDeath = 0

      for (const i of covidDataByCountry) {
        totalConfirmed += i.Confirmados
        totalDeath += i.Mortos
      }

      return { totalConfirmed, totalDeath }
    }
      ...
      if (country.length === 0) {
        ...
        xmlhttp.onreadystatechange = function () {
          if (this.readyState == 4 && this.status == 200) {
            ...
            const { totalConfirmed, totalDeath } = totalCasesByCountry(covidDataByCountry)
              ...
              document.querySelector(".fact--data").innerHTML += `
                <div class="fact__list">
                  ...
                  <span class="fact__list__item">
                    ...
                    <strong class="fact__list__item__copy--green">${formatNumber(i.Confirmados)}</strong>
                  </span>
                  <span class="fact__list__item">
                    ...
                    <strong class="fact__list__item__copy--red">${formatNumber(i.Mortos)}</strong>
                  </span>
                </div>
              `
            }

            document.querySelector(".fact__total").innerHTML = `
              <p class="fact__total--info--green"><strong>Total de casos confirmados:</strong> ${formatNumber(totalConfirmed)}</p>
              <p class="fact__total--info--red"><strong>Total de mortos:</strong> ${formatNumber(totalDeath)}</p>
            `
          }
        }
